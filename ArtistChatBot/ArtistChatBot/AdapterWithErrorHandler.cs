    ...
    public class AdapterWithErrorHandler : BotFrameworkHttpAdapter
    {
            ...
            : base(credentialProvider)
        {
            ...
            OnTurnError = async (turnContext, exception) =>
            {
                ...
                // Send a catch-all apology to the user. // "Something went wrong (BUGBUG: Fix ActivityBuilder.GenerateFromLG replacement" /*ActivityBuilder.GenerateFromLG(_lgEngine.EvaluateTemplate("SomethingWentWrong", exception)));
                await turnContext.SendActivityAsync(ActivityFactory.CreateActivity(_lgEngine.EvaluateTemplate("SomethingWentWrong").ToString());
